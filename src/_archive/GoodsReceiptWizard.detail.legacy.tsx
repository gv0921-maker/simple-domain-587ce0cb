/**
 * ARCHIVED — legacy Goods Receipt DETAIL path (rule 4: moved aside, not deleted).
 *
 * Verbatim copy of `src/pages/inventory/GoodsReceiptWizard.tsx` as it stood at
 * the GR detail rebuild (Pass 1). Nothing routes here.
 *
 * The route `/inventory/goods-receipts/:id` now renders
 * `src/pages/inventory/GoodsReceiptDetail.tsx`. The live wizard file is
 * UNTOUCHED and still serves `/inventory/goods-receipts/new` (create) until
 * Pass 2 rebuilds the create flow.
 *
 * What this file preserves — the behaviour the rebuild replaces:
 *   * `stepIndex` (line ~253) drove the whole detail view off `gr.status`.
 *   * At `stepIndex === 4` (completed/cancelled) it rendered ONLY a "Completed"
 *     card — the receipt's own fields were hidden once the GR was Done. That is
 *     the core defect Pass 1 fixes; keep it here for reference/restore.
 *
 * Restore by copying back over the live file. Do not edit this copy.
 */
