-- Batch 3: repair complete_ito_with_qc so a failed QC unit does not abort the
-- whole transfer completion.
--
-- The failure branch moved rejected serials to 'under_correction' but left
-- reserved_for_so_id populated. enforce_grs_reservation_status
-- (20260723124451) then raised "Cannot reserve serial % unless stock_status is
-- reserved" and rolled the transaction back, so the intended
-- 'blocked_by_failures' result — with its correction order — could never be
-- returned. The unit going to correction is no longer reserved for the SO, so
-- the reservation is now cleared in the same UPDATE.
--
-- Also replaces the opaque 'No passed units to move' error, which fired both
-- when nothing was scanned and when units were scanned but not yet marked
-- pass/fail, with a message that names which case it is.
--
-- Reproduced from 20260723144558 with only those two changes; approve_write_off
-- (redefined in that same migration) is intentionally not touched here — Batch 0
-- superseded it with the UUID-location + guard version.
